...
import com.wh.bdgw.MyApplication;
...
public class ToastUtils {
    ...
    private static int yOffset =
            (int) (64 * MyApplication.getApplication().getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setView(@LayoutRes int layoutId) {
        LayoutInflater inflate =
                (LayoutInflater) MyApplication.getApplication().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    private static void show(@StringRes int resId, int duration) {
        show(MyApplication.getApplication().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void show(@StringRes int resId, int duration, Object... args) {
        show(String.format(MyApplication.getApplication().getResources().getString(resId), args), duration);
    }
    ...
    private static void show(CharSequence text, int duration) {
        cancel();
        if (customView != null) {
            sToast = new Toast(MyApplication.getApplication());
            ...
            sToast = Toast.makeText(MyApplication.getApplication(), text, duration);
        }
